...
def register_combo_tools(mcp: FastMCP) -> None:
    ...
    async def list_combos(
        ...
        """List all stack combos, optionally filtered by ecosystem or deployment flavor. Returns pre-validated dependency sets."""
        limit = max(1, min(limit, 100))
        async with get_session_factory()() as session:
            repo = ComboRepository(session)
            combos = await repo.list_all(ecosystem=ecosystem, flavor=flavor, limit=limit)
            results = [
                {
                    "name": c.name,
                    "description": c.description,
