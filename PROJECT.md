...
- [x] (P1) Restore the broken Quality workflow contract: add a real `make check`, install/pin ruff and pyright in CI, select the project Python 3.12 environment deterministically, include greenlet for async SQLAlchemy tests, format/fix the repository, and replace the historical global lint baseline with explicit narrow exceptions for generated migrations, long MCP docstrings/seed data, and dynamic SQLAlchemy/FastMCP typing categories. Quality now runs ruff + format + pyright + 210 tests and is green locally. — resolved 2026-07-04
...
- [x] (P1) `onboard_app` now requires the approver key before validation, database access, or background scheduling, covering both direct approved-record creation and `replace_existing` de-escalation. A regression test proves an unapproved caller cannot reach the write path. Provenance: Fable adversarial review of WS-2.4's approver-gate PR. — added/resolved 2026-07-04
...
- [x] (P2) OpenRouter metadata is normalized before persistence in both app and open brains. Required keys, list element types, cardinality, thought type enum, and date shape are enforced; malformed-but-valid JSON degrades to complete safe defaults and unknown fields are dropped. Regression tests cover both brains. Provenance: Codex review 2026-03-24 finding #5. — added 2026-06-27; resolved 2026-07-04
